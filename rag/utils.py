...
logger = logging.getLogger("kgsrag")
...
def cal_list_token(list_data: list, key: callable):
    tokens = 0
    for i, data in enumerate(list_data):
        tokens += len(encode_string_by_tiktoken(key(data)))
    return list_data, tokens
...
def process_combine_all_contexts(input_texts):
    header = None
    list_texts = []
    for texts in input_texts:
        use_list = csv_string_to_list(texts.strip())
        if use_list:
            header = use_list[0]
            list_texts.append(use_list[1:])
    ...
    for texts in list_texts:
        str_texts.extend([",".join(item[1:]) for item in texts if item])

    combined_sources = []
    seen = set()
    for item in str_texts:
        if item and item not in seen:
            combined_sources.append(item)
            seen.add(item)
...
def process_combine_statements(input_texts):
    str_texts = []
    for item in input_texts:
        if item and len(item[1]) > 0 and item[1] != "UNKNOW":
            str_texts.append(",".join(item))
    combined_sources_result = ["id,\tstatement,\tanswer"]
    for i, item in enumerate(str_texts):
        combined_sources_result.append(f"{i},\t{item}")
    combined_sources_result = "\n".join(combined_sources_result)
    return combined_sources_result
