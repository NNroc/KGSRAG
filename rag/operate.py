...
async def kg_query(
    ...
    args_hash = compute_args_hash(query_param.decomposition, query_param.mode, query,
                                   query_param.min_threshold, query_param.max_threshold,
                                   query_param.alpha, query_param.beta)
...
async def _build_query_context(
    ...
    ll_keywords = ll_keywords[:query_param.alpha]
    hl_keywords = hl_keywords[:query_param.alpha]
...
async def _get_node_data(
    ...
    if query_param.mode == "all":
        use_filter_entities = await entities_vdb.filter(use_entities, statements_embeddings, 0.4, query_param)
        use_filter_relations = await relations_vdb.filter(use_relations, statements_embeddings, 0.2, query_param)
