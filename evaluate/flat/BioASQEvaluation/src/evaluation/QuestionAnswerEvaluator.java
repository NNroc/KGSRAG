    ...
    public void calculatePhaseBMeasuresForPair(Question golden, Question response) {
        if (question_type == Question.FACTOID) {
            if (this.VERSION_OF_CHALLENGE == EvaluatorTask1b.BIOASQ2) {
                ...
                if (golden.getExact_answer().getLists() == null && response.getExact_answer().getLists() == null) {
                    exact_answers.setStrict_accuracy(1.0);
                    exact_answers.setLenient_accuracy(1.0);
                    exact_answers.setMean_reciprocal_rank(1.0);
                    return;
                }
...
//                if (exact_answers.getStrict_accuracy() != exact_answers.getLenient_accuracy()){
//                    System.out.println();
//                }
//                System.out.println(golden.getId() + exact_answers.getStrict_accuracy() + exact_answers.getLenient_accuracy() + exact_answers.getMean_reciprocal_rank());
            }
    ...
    public void calculateMeasuresForPair(Question golden, Question response) {
        try {
            ...
            System.out.println(golden.getId());
        }
...
//        if(!golden.getConcepts().isEmpty())
    ...
    private void strictAccuracyForLists(ExactAnswer gold_answer, ExactAnswer system_answer, CalculatedMeasures exact_answers) {
        ...
        if (listsOfFactAnswers.isEmpty() || listsOfFactAnswers.get(0).isEmpty()) {
            exact_answers.setStrict_accuracy(0.0);
            return;
        }

        String firstElement = listsOfFactAnswers.get(0).get(0);
        ArrayList<String> newList = new ArrayList<>();
        newList.add(firstElement);

        if (gold_answer.containsAnswerSynonym(newList, false)) {
            exact_answers.setStrict_accuracy(1.0);
            return;
        }
    ...
    private void meanReciprocalRankForLists(ExactAnswer gold_answer, ExactAnswer system_answer, CalculatedMeasures exact_answers) {
        ...
        for (int i = 0; i < listsOfFactAnswers.get(0).size(); i++) {
            ArrayList<String> useList = new ArrayList<>();
            useList.add(listsOfFactAnswers.get(0).get(i));

            if (gold_answer.containsAnswerSynonym(useList, false)) {
                exact_answers.setMean_reciprocal_rank(1.0 / (double) (i + 1));
                return;
            }
        }
